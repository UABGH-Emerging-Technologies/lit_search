...
def get_step4_response(
    ...
    summarization.log_to_database(
        lit_app_config, content_to_log, start, finish, background_tasks, label="_scoping_step4"
    )
...
async def summarize_articles(
    background_tasks: BackgroundTasks, request: SummariesRequest, response: Response
) -> MSWordResponse:
    """
    ...
    response_data, warning_message = get_step4_response(
        background_tasks, request.research_question, request.xlsx_encoded
    )
