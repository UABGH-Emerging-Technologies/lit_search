...
def categorize(category_df, input_text):
    ...
    for index, row in reduced_df.iterrows():
        print(index)
        data = row['abstract']
        # Can we just use 3.5 for this? It just needs a category assignment....
        result = ScopingReview_config.FASTER_CHAT.invoke(ScopingReview_prompts.categorization_chat_prompt.format_prompt(categories=input_list, context=data).to_messages())
        # Unify category formatting
        out_string = result.content.replace("\"", "")
        out_string = out_string.replace("'", "")
        out_string = out_string.lower()
        reduced_df.at[index, 'category'] = out_string
    return reduced_df
