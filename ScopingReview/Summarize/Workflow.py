...
class SummarizeArticles(WorkflowHandler):
    def __init__(self, df, research_q):
        ...
        self.summarize_manager = SummarizeManager()
    ...
    def subcategorize(self, sub_categories):
        ...
        if categories_exceeding_limit:
            self.df, self.categories_str, response_meta = self.summarize_manager.sub_categorize(
    ...
    def summarize_articles(self) -> Tuple[bytes, dict, Optional[str]]:
        if self.df is not None:
            markdown_to_convert, response_meta = self.summarize_manager.summarize_all_categories(
    ...
    def write_newsletter(self, category, output_folder, template_location=None):
        if self.df is not None:
            newsletter_body, response_meta = self.summarize_manager.summarize_all_categories(
