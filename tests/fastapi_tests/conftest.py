...
def mock_external_calls(monkeypatch):
    ...
    try:
        ...
        monkeypatch.setattr(
            "ScopingReview_config.app_config.OPENAI_API_KEY", "test-key", raising=False
        )
    ...
    monkeypatch.setattr(
        "tiktoken.encoding_for_model", lambda *a, **k: DummyEncoding(), raising=False
    )
    ...
    monkeypatch.setattr(
        "aiweb_common.WorkflowHandler.WorkflowHandler._init_openai", mock_init_openai, raising=False
    )
