...
def show_literature_search_page():
    ...
    if query_type == "work on scoping review":
        scoping_step = st.radio(
            "What step of the scoping review do you want to work on?", scoping_steps
        )
        if (
            "prev_scoping_step" not in st.session_state
            or st.session_state["prev_scoping_step"] != scoping_step
        ):
            st.session_state["button_clicked"] = False
            st.session_state["search_finished"] = False
        ...
        if research_q == "":
            ...
            # Step 2: Iterate on search
                ...
                uploaded_file = st.file_uploader(
                    "Upload Excel File with Y/N selection", type=["xlsx"]
                )
                if not st.session_state.get("button_clicked", False):
            ...
            # Step 3: Categorize articles
                ...
                uploaded_file = st.file_uploader(
                    "Upload Excel File with Y/N selection for Categorization", type=["xlsx"]
                )
                ...
                if (
                    ...
                    and userdefined_categories.strip()
                    and userdefined_categories.strip().lower() != "category 1, category 2, etc..."
                ):
                    if not st.session_state.get("button_clicked", False):
                        if st.button("Run Categorization"):
                            finished = categorize_articles(
                                uploaded_file, userdefined_categories, research_q
                            )
                            if finished:
            ...
            # Step 5: Draft article
                ...
                uploaded_file = st.file_uploader(
                    "Upload document of summaries to draft scoping review", type=["docx"]
                )
            ...
            # Step 6: Generate bibtex file
                ...
                uploaded_file = st.file_uploader(
                    "Upload Finalized Excel sheet (CategorizeArticles.xlsx)", type=["xlsx", "docx"]
                )
